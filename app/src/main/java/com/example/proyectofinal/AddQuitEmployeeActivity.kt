...
class AddQuitEmployeeActivity : BaseActivity(), RadioGroup.OnCheckedChangeListener,
    ...
    private fun validateData() {
        ...
        if(name.isEmpty()){
            ...
            Snackbar.make(binding.root, "Debe seleccionar una sección", Snackbar.LENGTH_SHORT).show()
    ...
    private fun saveData(
        ...
        db.collection("users").orderBy("numEmple", Query.Direction.DESCENDING).get()
            .addOnSuccessListener { documents ->
                ...
                val pass = newEmpNum.toString() //Se genera la password igualandola al numero de empleado y se castea a String.
                auth.createUserWithEmailAndPassword(email, pass).addOnCompleteListener { task ->//Aqui metemos el user en authentication
                    if (task.isSuccessful) {
                        ...
                        val employeeNumber = hashMapOf("numEmple" to newEmpNum) //Hashmap para guardar el número de empleado en el documento "empleados_sala."
                        ...
                        val insertRegistered= db.collection("usuarios registrados").document(newEmpNum.toString()).set(registeredUser)
                        ...
                        if (job == "Jefe de sección") {
                            insertJob.collection("Jefe de sección").get()
                                .addOnSuccessListener { bossDocument ->
                                    if (bossDocument.isEmpty) {
                                         ...
                                         */
                                        Tasks.whenAllComplete(insertUser, insertRol, insertBoss,insertRegistered)
                                            .addOnSuccessListener {
                                                ...
                                                Snackbar.make(binding.root, "Error al guardar el jefe: ${e.message}", Snackbar.LENGTH_SHORT).show()
                                            }
                                    } else {
                                        Snackbar.make(binding.root, "$name $surname ya es el jefe de esta sección", Snackbar.LENGTH_SHORT).show()
                                    }
                                }
                        } else {
                             ...
                             */
                            Tasks.whenAllComplete(insertUser, insertRol, employeeInsert,insertRegistered)
                                .addOnSuccessListener {
...
private fun deleteUser(numEmple: String) {
    ...
    db.collection("users").document(numEmple).get().addOnSuccessListener { document ->
        if (document.exists()) {
            ...
            sectionNumber = when (findSection) { //Volvemos a darle valor a sectionNumber, ya que en esta cardView no hay un Spinner que proporcione el valor necesario
                "Sala" -> "1"
                "Charcutería" -> "2"
                "Pescadería" -> "3"
                "Frutería" -> "4"
            ...
            val setState = db.collection("usuarios registrados").document(numEmple).update("estado","inactivo")
            val deleteRol = db.collection("rol").document(rol).update("numEmple", FieldValue.delete()) //FieldValue.delete permite eliminar un campo de un documento mediante un UPDATE
            val locateSection = db.collection("secciones").document(sectionNumber)
            if (rol == "Jefe de sección") {
                val deleteBoss = locateSection.collection("Jefe de sección").document(numEmple).delete()

                Tasks.whenAllComplete(deleteUser, setState,deleteRol, deleteBoss).addOnSuccessListener {
                    Snackbar.make(binding.root, "Se ha borrado el jefe de sección correctamente", Snackbar.LENGTH_SHORT).show()
                }.addOnFailureListener { e ->
                    Snackbar.make(binding.root, "Error al borrar el jefe de sección ${e.message}", Snackbar.LENGTH_SHORT).show()
                }
            } else {
                val deleteEmployee = locateSection.collection("Empleados").document(numEmple).delete()
                Tasks.whenAllComplete(deleteUser, deleteRol, setState, deleteEmployee).addOnSuccessListener {
                    Snackbar.make(binding.root, "Se ha borrado el empleado correctamente", Snackbar.LENGTH_SHORT).show()
                }.addOnFailureListener { e ->
                    Snackbar.make(binding.root, "Error al borrar el empleado ${e.message}", Snackbar.LENGTH_SHORT).show()
                }
            }
        }
    }
